...
func Reader() {
	...
	for {
		...
		val, _ = rec.GetField(libnf.FldUsername)

		fmt.Println(val)
		// fmt.Println(err)

		val, _ = rec.GetField(libnf.FldIpNextHop)
		...
		val, _ = rec.GetField(libnf.FldSrcAS)
		...
		val, _ = rec.GetField(libnf.FldInSrcMac)
		...
		val, _ = rec.GetField(libnf.FldCalcBps)
		bps, ok := val.(float64)
		if !ok {
